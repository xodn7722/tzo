...
<body>
	...
	<%}  else {
			...
			%>
			<div class="col-md-3">
				<a href="contentRecruit.jsp?recruit_code=<%=vo.getRecruit_code()%>"><img src="images/<%=vo.getImg()%>" style="width: 100%; height: 70%"></a>
				<div>
	...
	if(count >0){
		...
		if(startPage > 10){ %>
			<a href="scroll.jsp?pageNum=<%=startPage -10 %>">[이전]</a>
		<% }
		for (int i = startPage; i<=endPage; i++){ %>
			<a href="scroll.jsp?pageNum=<%=i%>">[<%=i %>]</a>
	<%
		}
		  if (endPage < pageCount) {  %>
	        <a href="scroll.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
<% 
		  }
	}
%>

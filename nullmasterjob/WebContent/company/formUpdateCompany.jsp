...
<body>
	...
	<% } else {
		...
		CompanyDataBean vo = dao.getCompany(id);
		if (id.equals(vo.getEmail_id())){ %>
	<center>
	<div class="container" align="center">
		<div class="col-md-6 col-md-offset-4">
			<h3 class="form-signin-heading">회사 수정</h3>
			<form  name=form class="form-signin" action="processUpdate.jsp" method="post" >
				<div class="form-group">
					<label for="inputEventUrl" class="sr-only">회사 이름</label>
					<input type="text" class="form-control" value="<%=vo.getName()%>" name='name' required autofocus>
				</div>
				
				<div class="form-group">
					<label for="end_date" class="sr-only">회사 주소</label> 
					<input	type="text" class="form-control" value="<%=vo.getAddress() %>" name="address" required readonly>
					<input type="button" onClick="goPopup();" value="주소 찾기"/>
				</div>
				
				<div class="form-group">
					<label for="inputEmail" class="sr-only">회사 홈페이지</label> 
					<input	type="text" class="form-control" value="<%=vo.getHomepage() %>" name='homepage' required>
				</div>
				
				<div class="form-group">
					<label for="inputEmail" class="sr-only">설립날짜</label> 
					<input	type="text" class="form-control" value="<%=vo.getFound_date() %>" name='found_date' required>
				</div>
				
				<div class="form-group">
					<label for="inputEventUrl" class="sr-only">직원 수</label>
					<input type="text" class="form-control" value="<%=vo.getEmployee()%>"name='employee' required autofocus>
				</div>
				
				<div class="form-group">
					<label for="inputEventUrl" class="sr-only">회사소개</label>
					<textarea class="form-control" rows="15" cols="20" name="intro"><%=vo.getIntro() %></textarea>
				</div>
				<div class="form-group">
					...
					<input type="text" class="form-control" value="<%=id%>"name='email_id' readonly>
  						...
  						<button class="btn btn btn-lg btn-success btn-block" type="submit" onclick="return check()">수정</button>
			</form>
		</div>
	</div>
	<% 	}

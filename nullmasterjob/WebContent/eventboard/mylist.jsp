...
<tr>
   ...
   <% if(session.getAttribute("loginID") != null){ %> <a href="writeForm.jsp">글쓰기</a> <%}else{ %>
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="event_board.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
        <a href="event_board.jsp?pageNum=<%= i %>">[<%= i %>]</a>
<%
        }
        if (endPage < pageCount) {  %>
        <a href="event_board.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
<%
        }
    }
%>
